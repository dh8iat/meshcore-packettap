...
class QuestDBWriter:
    def __init__(self, host, port):
        self.host = host
        self.port = port
        self.queue = asyncio.Queue(maxsize=DB_QUEUE_MAXSIZE)
        self.flush_interval = DB_FLUSH_INTERVAL_SECONDS
        self.flush_row_threshold = DB_FLUSH_ROW_THRESHOLD
        self.running = True
        self.last_error_log = 0.0

    async def enqueue(self, table_name, ts_seconds, symbols, columns):
        if not WRITE_TO_DB or ts_seconds is None:
            return

        # QuestDB Line Protocol ist empfindlich gegen Steuerzeichen,
        # Zeilenumbrüche, ESC-Sequenzen und leere Symbolwerte.
        # Deshalb werden alle SYMBOLs als bereinigte Strings geschrieben.
        clean_symbols = {}
    ...
    def _send_batch_sync(self, batch):
        if not batch:
            return

        with Sender.from_conf(f"http::addr={self.host}:{self.port};") as sender:
            for item in batch:
                sender.row(
                    item["table_name"],
                    symbols=item["symbols"],
                    columns=item["columns"],
                    at=TimestampNanos(int(item["ts_seconds"]) * 1_000_000_000),
                )
            sender.flush()
    ...
    async def run(self):
        ...
        while self.running:
            try:
                timeout = max(0.1, self.flush_interval - (time.monotonic() - last_batch_time))
                ...
                if batch and (time.monotonic() - last_batch_time >= self.flush_interval):
                    should_flush = True

                if should_flush and batch:
                    await self._send_batch(batch)
                    log_verbose(f"[DB] Batch geschrieben: {len(batch)} Zeilen")
                    batch.clear()
                    last_batch_time = time.monotonic()
                    backoff = 1
                ...
                now = time.monotonic()
                if now - self.last_error_log >= DB_ERROR_LOG_INTERVAL_SECONDS:
                    print(f"[DB] Schreibfehler, Batch wird verworfen: {exc}")
                    self.last_error_log = now

                # Wichtig: Ein fehlerhafter Datensatz darf den gleichen Batch
                # nicht endlos erneut blockieren. Daher Batch verwerfen.
                batch.clear()
                last_batch_time = time.monotonic()
        ...
        # Restdaten beim Stop noch schreiben
        if batch:
            try:
                await self._send_batch(batch)
                log_verbose(f"[DB] Restbatch geschrieben: {len(batch)} Zeilen")
            except Exception as exc:
                print(f"[DB] Fehler beim Schreiben des Restbatches: {exc}")
...
def execute_questdb_sql(sql: str) -> bool:
    if not WRITE_TO_DB:
        return False

    try:
        base_url = f"http://{QUESTDB_HOST}:{QUESTDB_PORT}/exec"
        ...
        log_verbose(f"[QuestDB SQL] Fehler bei '{sql}': {exc}")
        return False


async def execute_questdb_sql_async(sql: str) -> bool:
    return await asyncio.to_thread(execute_questdb_sql, sql)


async def clear_contacts_snapshot():
    await execute_questdb_sql_async(f"TRUNCATE TABLE {TABLE_CONTACTS}")


async def write_row(table_name, ts_seconds, symbols, columns):
    global db_writer
    if db_writer is None:
        return
    await db_writer.enqueue(table_name, ts_seconds, symbols, columns)
...
async def write_mc_contact(
    ...
    await write_row(
        ...
        symbols={
            "public_key": public_key,
            "adv_name": adv_name,
            "contact_type": str(contact_type) if contact_type is not None else None,
            "out_path": out_path,
        },
        columns={
            "flags": flags,
...
async def main():
    ...
    try:
        if WRITE_TO_DB:
            db_writer = QuestDBWriter(QUESTDB_HOST, QUESTDB_PORT)
